...
const BasketForm = ({ basket, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        "Erro ao salvar produto. Verifique se preencheu todos os campos corretamente."
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-200">
        {/* Itens Inclusos */}
        <fieldset className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
          <legend className="text-sm font-semibold text-charcoal mb-4 flex items-center gap-2">
            <i className="bi bi-check2-circle text-terracotta"></i> Itens
            Inclusos na Cesta
          </legend>
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <input
              ...
              className="flex-1 p-3 border border-gray-200 rounded-xl text-sm focus:border-terracotta outline-none"
              onKeyPress={(e) =>
                e.key === "Enter" &&
                (e.preventDefault(), handleAddIncludedItem())
              }
            />
            <button
              type="button"
              onClick={handleAddIncludedItem}
              className="bg-charcoal text-white px-6 py-3 rounded-xl text-sm font-medium hover:bg-opacity-90 transition w-full sm:w-auto"
            >
              Adicionar
            </button>
          </div>
          <ul className="space-y-2 mt-2">
            {formData.includedItems.map((item, i) => (
              <li
                key={i}
                className="flex justify-between items-center bg-gray-50 p-3 px-4 rounded-xl border border-gray-100 text-sm"
              >
        ...
        {/* Itens Extras */}
        <fieldset className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mt-6 md:mt-0">
          <legend className="text-sm font-semibold text-charcoal mb-4 flex items-center gap-2">
            <i className="bi bi-plus-circle text-terracotta"></i> Opções de
            Extras (Upsell)
          </legend>
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <input
              ...
              className="flex-1 p-3 border border-gray-200 rounded-xl text-sm focus:border-terracotta outline-none"
              onKeyPress={(e) =>
                e.key === "Enter" && (e.preventDefault(), handleAddExtra())
              }
            />
            <input
              type="number"
              ...
              placeholder="R$ Preço"
              className="w-full sm:w-24 p-3 border border-gray-200 rounded-xl text-sm focus:border-terracotta outline-none"
              onKeyPress={(e) =>
                e.key === "Enter" && (e.preventDefault(), handleAddExtra())
              }
            />
            ...
            {formData.availableExtras.map((ex, i) => (
              ...
              >
                <span className="text-gray-700 font-medium">
                  {ex.name}{" "}
                  <span className="text-terracotta ml-1 font-bold">
