...
const OrderList = () => {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500"><i className="bi bi-arrow-clockwise animate-spin text-2xl"></i><br/>Carregando pedidos...</div>;
  ...
  return (
    <div className="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
      <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
        ...
        <button onClick={fetchOrders} className="text-sm text-terracotta hover:underline font-medium">
        ...
        <table className="min-w-full divide-y divide-gray-100">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Data / Período</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Cliente</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Resumo do Pedido</th>
            ...
            {orders.map((order) => (
              <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                  <div className="font-medium text-gray-900"><i className="bi bi-calendar-event"></i> {order.deliveryDate}</div>
                ...
                <td className="px-6 py-4 text-sm text-gray-600">
                  <div className="font-medium text-gray-900">{order.userId ? `Usuário #${order.userId}` : 'Convidado'}</div>
                    ...
                    {order.OrderItems?.map((item, idx) => {
                        ...
                        return (
                            <li key={idx} className="truncate max-w-xs">
                                <span className="font-medium text-gray-800">{item.quantity}x</span> {item.Basket?.name || `Produto #${item.basketId}`}
                                {safeExtras && safeExtras.length > 0 && (
                                    <span className="text-xs text-terracotta block ml-4">
                                        + {safeExtras.map(e => `${e.count}x ${e.name}`).join(', ')}
                                    </span>
                                )}
                            </li>
                        );
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                   <div className="text-sm font-bold text-charcoal">R$ {order.total}</div>
