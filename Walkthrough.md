...
3. Relatório de Entrega (Sprint Design & Expansão)
...
Implementada migration (change-image-to-longtext) para suportar armazenamento de imagens grandes (Base64) no banco de dados.
2. Home Page (/)
 Hero Section:
...
Ao passar o mouse, a imagem deve dar um leve "zoom" e o card deve subir um pouco (hover effects).
3. Página de Detalhes (/cesta/:id/:slug) (CORRIGIDO)
 Layout:
...
Role até o final da página. O rodapé deve estar colado no fundo, sem espaço branco extra abaixo dele.
 Funcionalidades:
O seletor de "Itens Extras" deve ter borda sutil e foco na cor Terracota.
